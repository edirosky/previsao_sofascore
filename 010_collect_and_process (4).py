...
def gerar_lista_de_datas_ontem_hoje() -> List[str]:
    """Gera lista de datas para ontem e hoje, considerando UTC e UTC-1"""
    agora_utc = datetime.now(timezone.utc)
    hoje_utc = agora_utc.date()
    ontem_utc = hoje_utc - timedelta(days=1)
    
    # Converter para UTC-1 (ajustar para o fuso horário da Europa Central, por exemplo)
    utc_minus_1 = timezone(timedelta(hours=-1))
    agora_utc_minus_1 = agora_utc.astimezone(utc_minus_1)
    hoje_utc_minus_1 = agora_utc_minus_1.date()
    ontem_utc_minus_1 = hoje_utc_minus_1 - timedelta(days=1)
    
    # Criar conjunto único de datas
    datas = set()
    datas.add(ontem_utc.strftime("%Y-%m-%d"))
    datas.add(hoje_utc.strftime("%Y-%m-%d"))
    datas.add(ontem_utc_minus_1.strftime("%Y-%m-%d"))
    datas.add(hoje_utc_minus_1.strftime("%Y-%m-%d"))
    
    # Ordenar as datas
    datas_ordenadas = sorted(list(datas))
    logging.info(f"Datas a buscar: {datas_ordenadas}")
...
def processar_evento(evento: Dict[str, Any], data_jogo: str) -> Dict[str, Any]:
    try:
        ...
        if start_timestamp:
            inicio_utc = datetime.fromtimestamp(start_timestamp, tz=timezone.utc)
        else:
            inicio_utc = None
            
        timestamp_evento = evento.get('timestamp')
        status_time_ts = evento.get('statusTime', {}).get('timestamp')
        
        if timestamp_evento:
            atual_utc = datetime.fromtimestamp(timestamp_evento, tz=timezone.utc)
        elif status_time_ts:
            atual_utc = datetime.fromtimestamp(status_time_ts, tz=timezone.utc)
        else:
            atual_utc = None
        ...
        return {
            ...
            'Inicio': inicio_utc.isoformat() if inicio_utc else None,
            'Atual_time': atual_utc.isoformat() if atual_utc else None,
...
async def coletar_eventos_playwright() -> List[Dict[str,Any]]:
    todos_eventos = []
    datas = gerar_lista_de_datas_ontem_hoje()
    headers = {
...
async def main_pipeline():
    ...
    # 1) collect events (busca entre ontem e hoje)
    eventos = await coletar_eventos_playwright()
    if eventos:
