...
# @title CARREGAMENTO DE MODELOS COMPLETO - VERSÃO FINAL OTIMIZADA
import os
import json
import joblib
import pandas as pd
...
def Carregar_modelos():
    ...
    print("=" * 60)
    print("🚀 INICIANDO CARREGAMENTO DE MODELOS - VERSÃO FINAL")
    print("=" * 60)
    ...
    if df.empty:
        print("❌ DataFrame vazio. Encerrando.")
        return
    ...
    print(f"\n📊 RESUMO FINAL:")
    print(f"   • Total de jogos: {len(df_final_check)}")
    ...
    if 'previsao_consensual_ajustada' in df_final_check.columns:
        ...
        print(f"   • PREVISÕES CONSENSUAIS SIM: {sim_consensual}/{len(df_final_check)}")
